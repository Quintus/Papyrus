...
RDoc::Generator::PDF_LaTeX class) files and a formatter for
...
* The generator uses LaTeX[http://latex-project.org] to generate the PDF files
  (pdfLaTeX, to be exact), so you’ll need this as well. LaTeX2e required.

== Installation

RDoc PDF LaTeX is available via RubyGems[http://rubygems.org].
Install it via:

  # gem install rdoc_pdf-latex

If you prefer to be on the bleeding edge, clone the sourcecode
repository and build the gem yourself:

  $ git clone http://git.github.com/Quintus/rdoc_pdf-latex
  $ rake gem
  # gem install --local --pre pkg/rdoc_pdf-latex-x.y.z

== Usage

To use it from the commandline, pass RDoc the <tt>-f</tt>
commandline switch specifying "pdf_latex":
...
program. Require it via:
  ...
  gem "rdoc", ">= 3.9" #Won’t work with any RDoc < 3.9
  require "rdoc/generator/pdf"

and then do

  opts = RDoc::Options.new
  #Specify your options...
  opts.generator = RDoc::Generator::PDF_LaTeX
...
[<tt>--show-pages</tt>]
  ...
  to true.
[<tt>--latex-command</tt>]
  If your pdfLaTeX’s executable for whatever reason isn’t named
  +pdflatex+ (the default), you can set the name here.
[<tt>--babel-lang</tt>]
...
* The sourcecode is hosted at GitHub: https://github.com/Quintus/rdoc_pdf-latex
* Report bugs at: https://github.com/Quintus/rdoc_pdf-latex/issues
...
of {Pegasus Alpha}[http://pegasus-alpha.de], which disclaims all
interest on the RDoc PDF LaTeX plugin. You can contact me at
m-guelker ÄT pegasus-alpha DÖT de.
